...
public class CalcRomeArab {
    ...
    public String result(String exp) throws CalcExcept {
        ...
        if (result < 0 && isRomanExp == isRomanExp) {      // т.к римские числа не могут быть отрицательными
                                                           // сделаем проверку
            throw new CalcExcept("ERROR. Римские числа не могут быть отрицатальными");
        }

        // сделаем проверку если числа римские, то конвертируем в арабские и возвращаем результат

        if (isRomanExp){
            String sign = result < 0 ? "-" : "";
            return sign + parse.arabicRomeConverter(Math.abs(result));
        }
